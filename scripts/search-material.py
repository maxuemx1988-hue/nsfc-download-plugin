...
Usage:
    ...
    # Uses MATERIAL_KEYWORDS from config.py
"""
import sys
import os
...
from lib.config_loader import MATERIAL_KEYWORDS, NSFC_HOST, CDP_PORT
...
def search_material_keywords(client):
    if not MATERIAL_KEYWORDS:
        print("No MATERIAL_KEYWORDS configured in config.py.")
        return []
    ...
    _navigate_and_wait(MATERIAL_KEYWORDS[0])
...
def main():
    print("=" * 60)
    print("NSFC Search - Material Keywords")
    print(f"Keywords: {MATERIAL_KEYWORDS}")
    print("=" * 60)
    ...
    print("\n[2/2] Searching...")
    projects = search_material_keywords(client)
    print(f"\n{'='*60}")
