...
Usage:
    ...
    # Uses SUB_KEYWORDS from config.py
"""
import sys
import os
...
from lib.config_loader import SUB_KEYWORDS, NSFC_HOST, CDP_PORT, MAX_RESULTS_THRESHOLD
...
def search_sub_keywords(client):
    if not SUB_KEYWORDS:
        print("No SUB_KEYWORDS configured in config.py.")
        return []
    ...
    def _fresh_search(keyword, filters, filter_info, full_count=None, retry=True):
        nonlocal seen_ids
        import time
        collected = []
        ...
        if retry and full_count and len(filters) > 0 and total >= full_count:
            return _fresh_search(keyword, filters, filter_info, full_count, retry=False)
    ...
    _navigate_and_wait(SUB_KEYWORDS[0])
    ...
    for kw in SUB_KEYWORDS:
        ...
        if total > MAX_RESULTS_THRESHOLD:
            ...
            for yr_opt in usable_years:
                ...
                if yr_total > MAX_RESULTS_THRESHOLD and yr_total < full_count:
...
def main():
    print("=" * 60)
    print("NSFC Search - Sub Keywords")
    print(f"Keywords ({len(SUB_KEYWORDS)}): {SUB_KEYWORDS}")
    print("=" * 60)
    ...
    print("\n[2/2] Searching...")
    projects = search_sub_keywords(client)
    print(f"\n{'='*60}")
